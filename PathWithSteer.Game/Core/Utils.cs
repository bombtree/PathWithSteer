...
{
    public enum CustomCollisionFilterGroups
    {
        ground = CollisionFilterGroups.CustomFilter1,
        interactables = CollisionFilterGroups.CustomFilter2,
        navObstacles = CollisionFilterGroups.CustomFilter3
    }
    
    public static class Utils
    {
        public static CollisionFilterGroups Value(this CustomCollisionFilterGroups customCollisionFilter)
        {
            return (CollisionFilterGroups)customCollisionFilter;
        }
        ...
        public static StartEndRay ScreenPositionToWorldRay(Vector2 screenPos, CameraComponent camera)
        {
            Matrix invViewProj = Matrix.Invert(camera.ViewProjectionMatrix);
            StartEndRay startEndRay = new StartEndRay();
            ...
            sPos.Z = 0f;
            var rayStartVec4 = Vector3.Transform(sPos, invViewProj);
            startEndRay.rayStart = rayStartVec4.XYZ() / rayStartVec4.W;

            sPos.Z = 1f;
            var rayEndVec4 = Vector3.Transform(sPos, invViewProj);
            startEndRay.rayEnd = rayEndVec4.XYZ() / rayEndVec4.W;
            return startEndRay;
        }
        ...
        {
            StartEndRay startEndRay = ScreenPositionToWorldRay(screenPos, camera);
            ...
            simulation.RaycastPenetrating(startEndRay.rayStart, startEndRay.rayEnd, result, hitTriggers: true);
            foreach (var hitResult in result)
            {
                ...
                if (staticBody != null)
                {
                    ...
                    {
                        var distance = (startEndRay.rayStart - hitResult.Point).LengthSquared();
        ...
        public static Vector3 ClampLength(this Vector3 vector3, float maxLength)
        {
            if(vector3.LengthSquared() >=  maxLength * maxLength)
            {
                return Vector3.Normalize(vector3) * maxLength;
            }
            else
            {
                return vector3;
        ...
        public static Vector3 VelocityInUnitsPerSecond(this CharacterComponent character)
        {
        ...
        public static List<T> Shuffle<T>(this List<T> list, Random random)
        {
            ...
            for (int itemsToShuffle = list.Count - 1; itemsToShuffle >= 1; --itemsToShuffle) 
            {
                int index = random.Next(0, itemsToShuffle);
                swap = list[itemsToShuffle];
                list[itemsToShuffle] = list[index];
                list[index] = swap;
            }
            return list;
        } 
        ...
        public static void SetWorldPosition(this Entity entity, Vector3 position)
        {
            entity.Transform.WorldMatrix.TranslationVector = position;
        }
    }
}
